          ...
          <TableBody>
            {customerData.map((row, i) => (
              <TableRow key={row._id.toString()} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                <TableCell>
                  <Checkbox onClick={() => handleUserEmailData(row.email)} checked={userEmailData.includes(row.email)} />
                </TableCell>
                ...
                <TableCell align='right'>
                  {row?.address?.length ? row.address.map((add,i)=>(
                  <p>Address{i+1}{": "}{add?.line1+", "+add?.line2+", "+add?.state+", "+add?.pin}</p>
                  )): row.address ? (row.address?.line1+", "+row.address?.line2+", "+row.address?.state+", "+row.address?.pin): "no address found" }
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
