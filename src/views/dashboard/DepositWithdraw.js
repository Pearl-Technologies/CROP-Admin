    ...
    <Card sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: ['column', 'column', 'row'] }}>
      <Box sx={{ width: '100%' }}>
        ...
        <CardContent sx={{ pb: theme => `${theme.spacing(5.5)} !important` }}>
          {accountTrasaction.map((item, index) => {
            if(item.transactionType === 'credit'){            
            return (
              <Box
                key={item._id}
                sx={{ display: 'flex', alignItems: 'center', mb: index !== accountTrasaction.length - 1 ? 6 : 0 }}
              >
                <Box sx={{ minWidth: 38, display: 'flex', justifyContent: 'center' }}>
                  <img src={"/images/logos/stripe.png"} alt={item.title} width={28} height={20} />
                </Box>
                <Box
                  sx={{
                    ml: 4,
                ...
                >
                  <Box sx={{ marginRight: 2, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Typography variant='caption'>{new Date(item.createdAt).toDateString()}</Typography>
            ...
            return (
              ...
              >
                <Box sx={{ minWidth: 36, display: 'flex', justifyContent: 'center' }}>
                  <img src={'/images/logos/mastercard-label.png'} alt={item.title} width={38} height={38} />
                </Box>
                <Box
                  sx={{
                    ml: 4,
